    ...
    <Card className="w-full h-80 flex flex-col justify-between overflow-hidden relative pb-12 border border-muted shadow-sm hover:shadow-lg transition-shadow duration-200 bg-white dark:bg-neutral-900">
      <div className="relative flex-shrink-0">
        {product.image_url ? (
          <img
            ...
            alt={product.name}
            className="w-full h-44 object-contain rounded-t-md bg-white dark:bg-black border-b border-muted"
          />
        ) : (
          <div className="w-full h-44 bg-gray-100 dark:bg-neutral-800 flex items-center justify-center text-sm text-muted-foreground rounded-t-md border-b border-muted">
            No image
          </div>
        )}
      ...
      <CardContent className="flex-1 flex flex-col justify-between px-4 py-2">
        <div>
          ...
          <div className="mt-1 text-lg font-bold line-clamp-2 text-foreground">{product.name}</div>
        </div>
      </CardContent>
